...
1. **제로폭/불가시 유니코드 + null 바이트 선제거**
2. **완전 디코딩**: URL 인코딩이 수렴할 때까지 `rawurldecode` 반복 (최대 10회)
3. **오버롱 UTF-8 바이너리 제거**: `\xc0\xae`, `\xe0\x80\xae`, `\xf0\x80\x80\xae` 등 + Tomcat `..;`
4. **경로 구분자 정규화**: `\` → `/` 통일
5. **반복 치환**: `../` 패턴이 완전히 소멸할 때까지 while 루프
6. **세그먼트 기반 최종 검증**: `explode('/')` 후 `..` 세그먼트 완전 제거 (`test/..`, 단독 `..` 포함)
