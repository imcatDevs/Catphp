# 보안 — Auth · Csrf · Encrypt · Firewall · Ip · Guard · Sanitizer

CatPHP의 보안 계층. 인증, CSRF 보호, 암호화, IP 차단, 입력 살균, HTML 정제를 제공한다.

| 도구 | Shortcut | 클래스 | 줄 수 |
| --- | --- | --- | --- |
| Auth | `auth()` | `Cat\Auth` | 267 |
| Csrf | `csrf()` | `Cat\Csrf` | 79 |
| Encrypt | `encrypt()` | `Cat\Encrypt` | 85 |
| Firewall | `firewall()` | `Cat\Firewall` | 211 |
| Ip | `ip()` | `Cat\Ip` | 195 |
| Guard | `guard()` | `Cat\Guard` | 541 |
| Sanitizer | `sanitizer()` | `Cat\Sanitizer` | 1041 |
...
7. [Sanitizer — HTML 정제/XSS 방어](#7-sanitizer--html-정제xss-방어)
...
## 7. Sanitizer — HTML 정제/XSS 방어

WYSIWYG 에디터, 댓글, 게시판 등 사용자 입력 HTML에서 XSS 공격 벡터를 제거. DOMDocument 기반 7단계 보안 레이어로 195개 이상의 공격 패턴 방어.
...
| mXSS | Mutation XSS (noscript, noembed, xmp) |
| SVG/MathML | 애니메이션 이벤트, xlink:href |
| DOM Clobbering | 위험 id/name 속성 (document, location 등) |
...
- `on*` (모든 이벤트 핸들러)
- `style`, `srcdoc`, `formaction`, `action`
- `xlink:href`, `xmlns`
- `autofocus`, `contenteditable`, `ping`

### 보안 상수

| 상수 | 개수 | 설명 |
| --- | --- | --- |
| `DANGEROUS_TAGS` | 30 | 위험 태그 (내용까지 제거) |
| `DANGEROUS_ATTRS` | 25 | 위험 속성 |
| `DANGEROUS_PROTOCOLS` | 11 | 위험 프로토콜 (javascript:, data: 등) |
| `DANGEROUS_IDS` | 60 | DOM Clobbering 방어 ID |

> 상세 문서: [Sanitizer](tools/Sanitizer.md)
